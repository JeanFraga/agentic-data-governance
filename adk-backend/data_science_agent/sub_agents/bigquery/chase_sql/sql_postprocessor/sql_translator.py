...
"""Translator from SQLLite to BigQuery."""
...
def _isinstance_list_of_str_tuples_lists(obj: Any) -> bool:
    """Checks if the object is a list of tuples or lists of strings."""
    return (
        isinstance(obj, list)
...
def _isinstance_ddl_schema_type(obj: Any) -> bool:
    ...
    return (
        ...
        and all(
            # Every element above list is a tuple or list of strings
...
def _isinstance_bird_sample_type(obj: Any) -> bool:
    """Checks if the object is a Bird sample type."""
...
class SqlTranslator:
    ...
    1.  (Optional) If there are errors in the input SQL query, the input SQL query
        is the first modified by the LLM to address the errors.
    ...
    GEMINIMODEL: Final[str] = os.environ.get(
        "SQL_TRANSLATOR_MODEL", "gemini-2.0-flash-001"  # Default model
    )

    def __init__(
            self,
            model: str | GeminiModel = GEMINIMODEL,
    ...
    def _extract_schema_from_ddl_statement(cls, ddl_statement: str) -> TableSchemaType:
        ...
        # Match the following pattern:
        # <column_name> <column_type> [<ignored_test>]
        # [, <column)name> <column_type> [<ignored_text>]]*
        column_pattern = (
            # Ignoere any comments.
            r"\s*--.*(*SKIP)(*FAIL)"
            ...
            r"|\s*(?:`)?\s*(?P<column_name>\w+)(?:`)?\s+(?<column_type>\w+).*"
        ) # (?:,)?
    ...
    def _get_schema_from_bird_sample(
        ...
        for id_pos, table_id in enumerate(table_ids):
            if tables[table_id] in tables_to_columns.keys():
                tables_to_columns[tables[table_ids]].update(
    ...
    def _get_table_parts(cls, table_name: str) -> tuple[str | None, str | None, str]:
        """Returns the table parts from table names."""
