...
from data_science_agent.utils.utils import get_env_var
...
def update_database_settings():
    ...
    database_settings = {
        ...
        # Inlcude ChaseSQL-specific constants.
...
def get_bigquery_schema(dataset_id, client=None, project_id=None):
    ...
    Args:
        ...
        project_id (str): The ID of your Google Cloud Projects.
...
def initial_bq_nl2sql(
    ...
    """Generate an initial SQL query from natural language question.
...
- **FILTERS:** You should write query effectively to reduce and minimize the total rows to be returned. For example, you can use filters (like `WHERE`, `HAVING`, etc.) and aggregations (like 'COUNT', 'SUM', etc.) in the SQL query.
...
**Think Step-byStep:** Carefully consider the schema, question, guidelines, and best practices outlined above to generate the correct BigQuery SQL.
...
def run_bigquery_validation(
    ...
    # More restrictive check for BigQuery - disallow DML and DLL
    if re.search(
